...
// Cloud Bunny Volumetric Rendering
//*********************************************************
...
namespace
{
    const wchar_t* kTetPathCandidates[] = {
        L"..\\..\\..\\..\\Assets\\IntervalShading\\bunny.vtk",
        L"..\\..\\..\\Assets\\IntervalShading\\bunny.vtk",
        L"..\\Assets\\IntervalShading\\bunny.vtk",
        L"Samples\\Desktop\\D3D12MeshShaders\\src\\Assets\\IntervalShading\\bunny.vtk"
    };
    constexpr float kDefaultDensity = 6.0f;
...
void IntervalShadingTetrahedron::LoadPipeline()
{
    ...
    // Render targets and command allocators
    {
...
void IntervalShadingTetrahedron::LoadAssets()
{
    ...
    for (auto path : kTetPathCandidates) {
        if (LoadTetrahedralMesh(path)) { loaded = true; break; }
    }
    ...
    ThrowIfFailed(m_device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT,
        m_commandAllocators[m_frameIndex].Get(), m_intervalPipelineState.Get(),
    ...
    // Constant buffer
    {
        ...
        CD3DX12_RESOURCE_DESC bufferDesc = CD3DX12_RESOURCE_DESC::Buffer(m_cbStride * FrameCount);
        ThrowIfFailed(m_device->CreateCommittedResource(&uploadHeap, D3D12_HEAP_FLAG_NONE,
            &bufferDesc, D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&m_constantBuffer)));
...
void IntervalShadingTetrahedron::UpdateConstants()
{
    ...
    XMVECTOR lightDir = XMVector3Normalize(XMVectorSet(0.5f, 0.8f, -0.3f, 0.0f));
    XMStoreFloat3(&m_constantBufferData.LightDir, lightDir);
    m_constantBufferData.LightColor = XMFLOAT3(5.0f, 4.5f, 4.0f);
    m_constantBufferData.SigmaS = XMFLOAT3(0.95f, 0.95f, 0.98f);
    m_constantBufferData.G = 0.75f;
    m_constantBufferData.DebugMode = 5;
...
void IntervalShadingTetrahedron::PopulateCommandList()
{
    ...
    // Pass 1: Interval Shading
    {
        ...
        m_commandList->OMSetRenderTargets(3, rtHandles, FALSE, nullptr);
    ...
    // Pass 2: Composite
    {
        ...
        const float clearColor[] = { 0.0f, 0.02f, 0.05f, 1.0f };
...
void IntervalShadingTetrahedron::OnKeyDown(UINT8 key)
{
    ...
    case '4': m_constantBufferData.DebugMode = 4; break;
    case '5': m_constantBufferData.DebugMode = 5; break;
    ...
    if (m_cameraAngle > XM_2PI) m_cameraAngle -= XM_2PI;
    if (m_cameraAngle < 0) m_cameraAngle += XM_2PI;
    if (m_cameraElevation > XM_2PI) m_cameraElevation -= XM_2PI;
    if (m_cameraElevation < 0) m_cameraElevation += XM_2PI;
}
...
{
    if (!m_tetBufferMapped || m_tetIndices.empty()) return;
...
bool IntervalShadingTetrahedron::LoadTetrahedralMesh(const std::wstring& path)
{
    ...
    XMVECTOR minV = XMLoadFloat3(&minP), maxV = XMLoadFloat3(&maxP);
...
void IntervalShadingTetrahedron::CreateIntervalTargets()
{
    ...
    auto frontDesc = CD3DX12_RESOURCE_DESC::Tex2D(DXGI_FORMAT_R16_FLOAT, m_width, m_height, 1, 1, 1, 0, D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET);
    ...
    ThrowIfFailed(m_device->CreateCommittedResource(&heap, D3D12_HEAP_FLAG_NONE, &frontDesc, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE, &frontClear, IID_PPV_ARGS(&m_frontRT)));
    CD3DX12_CPU_DESCRIPTOR_HANDLE frontHandle(m_rtvHeap->GetCPUDescriptorHandleForHeapStart(), FrameCount, m_rtvDescriptorSize);
    ...
    backClear.Color[0] = 0.0f;
    ThrowIfFailed(m_device->CreateCommittedResource(&heap, D3D12_HEAP_FLAG_NONE, &backDesc, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE, &backClear, IID_PPV_ARGS(&m_backRT)));
    CD3DX12_CPU_DESCRIPTOR_HANDLE backHandle(m_rtvHeap->GetCPUDescriptorHandleForHeapStart(), FrameCount + 1, m_rtvDescriptorSize);
    m_device->CreateRenderTargetView(m_backRT.Get(), nullptr, backHandle);
    m_backState = D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE;

    auto opticalDesc = CD3DX12_RESOURCE_DESC::Tex2D(DXGI_FORMAT_R16_FLOAT, m_width, m_height, 1, 1, 1, 0, D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET);
    D3D12_CLEAR_VALUE opticalClear = {};
    opticalClear.Format = opticalDesc.Format;
    opticalClear.Color[0] = 0.0f;
    ThrowIfFailed(m_device->CreateCommittedResource(&heap, D3D12_HEAP_FLAG_NONE, &opticalDesc, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE, &opticalClear, IID_PPV_ARGS(&m_opticalDepthRT)));
    CD3DX12_CPU_DESCRIPTOR_HANDLE opticalHandle(m_rtvHeap->GetCPUDescriptorHandleForHeapStart(), FrameCount + 2, m_rtvDescriptorSize);
...
void IntervalShadingTetrahedron::BuildIntervalPipelineState()
{
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        blend.RenderTarget[i].DestBlend = D3D12_BLEND_ONE;
...
void IntervalShadingTetrahedron::BuildCompositePipelineState()
{
    ...
    CD3DX12_DESCRIPTOR_RANGE1 range;
    range.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 3, 2);
    CD3DX12_ROOT_PARAMETER1 params[2];
    params[0].InitAsConstantBufferView(0);
...
void IntervalShadingTetrahedron::UploadBuffer(ID3D12Resource** destination, const void* data, size_t byteSize)
{
    ...
    ThrowIfFailed(m_device->CreateCommittedResource(&heap, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(destination)));
